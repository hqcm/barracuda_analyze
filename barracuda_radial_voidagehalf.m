...
radialvoidage=zeros(size(b,1),2);
j=1;m=0;
for i=1:size(a,1)
    if (abs(a(i,1)-b(j))<e2)
        radialvoidage(j,1:2)=radialvoidage(j,1:2)+a(i,1:2);
    else
        radialvoidage(j,1:2)=radialvoidage(j,1:2)/m;
        j=j+1;m=0;
        radialvoidage(j,1:2)=radialvoidage(j,1:2)+a(i,1:2);
    end
    m=m+1;
...
radialvoidage(all(radialvoidage==0,2),:)=[];
