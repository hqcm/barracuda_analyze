...
while ~feof(fid)                                  % 判断是否为文件末尾
    ...
    if contains(tline, 'Pressure')
        Pressure=str2double(tline(2));  %得到空隙率所在的列数
    end
...
data2=[line1;data1'];
data=data2(:,[2 Pressure Tcolnumber]);  %提取列数据
data(:,3)=data(:,3)-(-0.102081);  %将z的起始点设为0点
x_t=2.4974;x_b=0.0026;%设定提升管的最低点和最高点
...
save(char(str), 'pressuredrop','-ascii');  %注意save函数的格式
